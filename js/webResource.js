 ...
 * Original file: /npm/@wppconnect/wa-js@3.17.7/dist/wppconnect-wa.js
 *
...
async showNeedsReply() {
  let dataArr = WAM.whatsapp.ChatStore._models;
  // dataArr.forEach((data) => {
  //     data.__x_shouldAppearInList = false;
  //     data.t = data.t + 1;
  // });
  let filteredData = dataArr.filter(
    (data) => data.lastReceivedKey && !data.lastReceivedKey.fromMe
  );
  await WAM.chat.setChatList(
    "custom",
    filteredData.map((a) => a.id._serialized)
  );
  // filteredData.forEach((data) => {
  //     data.__x_shouldAppearInList = true;
  //     data.t = data.t - 1;
  // });
  // console.log("done");
}
...
async showGroups() {
  ...
  //     data.t = data.t + 1;
  // });
  let filteredData = dataArr.filter((data) => data.isGroup);
  await WAM.chat.setChatList(
    "custom",
    filteredData.map((a) => a.id._serialized)
  );
...
async showUnread() {
  ...
  //     data.t = data.t + 1;
  //   });
  let unreadMsgs = dataArr.filter((data) => data.hasUnread);
  await WAM.chat.setChatList(
    "custom",
    unreadMsgs.map((a) => a.id._serialized)
  );
  //   unreadMsgs.forEach((data) => {
  //     data.__x_shouldAppearInList = true;
  //     data.t = data.t - 1;
  //   });
  console.log("done");
...
async showPersonalAcc() {
  ...
  //     data.t = data.t + 1;
  // });
  let filteredData = dataArr.filter((data) => data.isUser);
  await WAM.chat.setChatList(
    "custom",
    filteredData.map((a) => a.id._serialized)
  );
...
async showBusinessAcc() {
  ...
  //     data.t = data.t + 1;
  // });
  let filteredData = dataArr.filter((data) => data.contact.isBusiness);
  await WAM.chat.setChatList(
    "custom",
    filteredData.map((a) => a.id._serialized)
  );
...
async showMentioned() {
  ...
  //     data.t = data.t + 1;
  // });
  let filteredData = dataArr.filter((data) => data.hasUnreadMention);
  await WAM.chat.setChatList(
    "custom",
    filteredData.map((a) => a.id._serialized)
  );
...
async showOfficialAcc() {
  ...
  //     data.t = data.t + 1;
  // });
  let filteredData = dataArr.filter((data) => data.contact.isEnterprise);
  await WAM.chat.setChatList(
    "custom",
    filteredData.map((a) => a.id._serialized)
  );
...
async showAll() {
  ...
  await WAM.chat.setChatList(
    "custom",
    dataArr.map((a) => a.id._serialized)
  );
}

checkArchive(chatId) {
  let dataArr = WAM.whatsapp.ChatStore._models;
  let filteredData = dataArr.filter((data) => data.id.user === chatId);
  return filteredData[0].archive;
}
...
getContacts(chatId) {
  ...
  // console.log(chatId);
  // console.log(filteredData[0].isGroup);

  if (filteredData[0].groupMetadata) {
    let contacts = filteredData[0].groupMetadata.participants._models;
    // console.log(contacts);

    let filteredContacts = contacts.map((ele) => {
      let name =
        WAM.whatsapp.ContactStore.get(ele.id._serialized).name ||
        WAM.whatsapp.ContactStore.get(ele.id._serialized).verifiedName ||
        WAM.whatsapp.ContactStore.get(ele.id._serialized).pushname ||
        ele.id.user;
      let img = WAM.whatsapp.ProfilePicThumbStore.get(
        ele.id._serialized
      )?.img;
      return { name, number: ele.id.user, image: img };
    });
    return filteredContacts;
  }
  let img = WAM.whatsapp.ProfilePicThumbStore.get(
    filteredData[0].id._serialized
  ).img;
  return [
    {
      name: filteredData[0].formattedTitle,
      number: filteredData[0].id.user,
      image: img,
    },
  ];
}
...
getContact(number) {
  let dataArr = WAM.whatsapp.ChatStore._models;
  let serialized = number + "@c.us";
  let contact = WAM.whatsapp.ContactStore.get(serialized);
  let name =
    contact.name ||
    contact.verifiedName ||
    contact.pushname ||
    contact.id.user;
  let img = WAM.whatsapp.ProfilePicThumbStore.get(serialized)?.img;
  return [{ name, number, image: img }];
}

archiveChat(chatId) {
  let dataArr = WAM.whatsapp.ChatStore._models;
  let filteredData = dataArr.filter((data) => data.id.user === chatId);
  filteredData[0].archive = true;
}
getChatNumData(chatIds) {
  let dataArr = WAM.whatsapp.ChatStore._models;
  let starred = dataArr.filter(
    (data) => chatIds.includes(data.id.user) && data.unreadCount > 0
  );
  let offical = dataArr.filter(
    (data) => data.contact.isEnterprise && data.unreadCount > 0
  );
  let business = dataArr.filter(
    (data) => data.contact.isBusiness && data.unreadCount > 0
  );
  let mentioned = dataArr.filter((data) => data.hasUnreadMention);
  let personal = dataArr.filter(
    (data) => data.isUser && data.unreadCount > 0
  );
  let groups = dataArr.filter((data) => data.isGroup && data.unreadCount > 0);
  let unread = dataArr.filter((data) => data.hasUnread);
  let awaiting = dataArr.filter(
    (data) => data.lastReceivedKey && data.lastReceivedKey.fromMe
  );
  let needsReply = dataArr.filter(
    (data) => data.lastReceivedKey && !data.lastReceivedKey.fromMe
  );
  return {
    all: dataArr.length,
    starred: starred.length,
    official: offical.length,
...
unArchiveChat(chatId) {
  let dataArr = WAM.whatsapp.ChatStore._models;
  let filteredData = dataArr.filter((data) => data.id.user === chatId);
  filteredData[0].archive = false;
}

async customChats(chatIds) {
  let dataArr = WAM.whatsapp.ChatStore._models;
  // dataArr.forEach((data) => {
  //     data.__x_shouldAppearInList = false;
  //     data.t = data.t + 1;
  ...
  let filteredData = dataArr.filter((data) => chatIds.includes(data.id.user));
  await WAM.chat.setChatList(
    "custom",
    filteredData.map((a) => a.id._serialized)
  );
...
async inboxChats(closedChats, snoozedChats) {
  // console.log(closedChats,snoozedChats);
  ...
  // let filteredClosedData = dataArr.filter((data) => !);
  let filteredData = dataArr.filter(
    (data) =>
      !(
        snoozedChats.includes(data.id.user) ||
        closedChats.includes(data.id.user)
      )
  );
  await WAM.chat.setChatList(
    "custom",
...
async sendAttachment(data) {
  ...
  const file = data.attachment.fileData;
  let caption = data.attachment.fileCaption;
  caption = caption.trim();

  const fileData = await JSON.parse(file);
  if (
    data.attachment.fileAddTimeStamp == undefined ||
    data.attachment.fileAddTimeStamp
  ) {
  ...
  try {
    await WAM.chat.sendFileMessage(chatId, fileData, {
      type: "auto-detect",
      caption: caption ? caption : null,
      filename: data.attachment.fileName,
      // mimetype: data.attachment.fileType,
    });
...
async filterNumber(data) {
  const chatId = data.number + "@c.us";
  const result = await WAM.contact.queryExists(chatId);
  window.dispatchEvent(
    new CustomEvent("WAM::filtered", {
      detail: { status: result ? "Valid" : "Invalid" },
    })
  );
}
...
listenForMessages() {
  window.addEventListener("message", async (event) => {
    ...
    // console.log("Web Resource received a message:", event.data.message);

    switch (event.data.message.request) {
      case "getChatNumData":
        let data = this.getChatNumData(event.data.message.chatIds);
        // console.log(data);
        ...
        break;
      case "custom":
        this.customChats(event.data.message.chatIds);
        break;
      case "Inbox":
        this.inboxChats(
          event.data.message.closedChats,
          event.data.message.snoozedChats
        );
        break;
      case "snoozed":
        this.starredChats(event.data.message.chatIds);
        break;
      case "starred":
        this.starredChats(event.data.message.chatIds);
        break;
      case "closed":
        ...
        break;
      case "UnArchiveChat":
        this.unArchiveChat(event.data.message.chatId);
        break;
      case "archiveChat":
        this.archiveChat(event.data.message.chatId);
        break;
      case "checkArchive":
        let res = this.checkArchive(event.data.message.chatId);
        this.sendMessageToContentScript({
          response: res,
          for: "checkArchive",
        });
      ...
      case "getLinkedContacts":
        let response = this.getContact(event.data.message.number);

        let contact = event.data.message.contact;
        // console.log(contact);
        this.sendMessageToContentScript({
          contact,
        ...
        break;
      case "getContacts":
        let resp = this.getContacts(event.data.message.chatId);
        this.sendMessageToContentScript({
        ...
        break;
      case "createContact":
        let respo = this.getContacts(event.data.message.contactId);
        this.sendMessageToContentScript({
          response: JSON.stringify(respo),
          for: "createContact",
        });
        break;
      case "unread":
      ...
      case "groups":
        ...
        break;
      case "sendAttachment":
        this.sendAttachment(event.data.message);
        break;
      case "filterNumber":
        this.filterNumber(event.data.message);
        break;
    ...
    // setTimeout(async () => {
    //   this.sendMessageToContentScript({ response: "complete" });
    // }, 5000);
    // console.log(WAM.whatsapp.ChatStore._models[0]);
    // this.showUnread();
  });
}

sendMessageToContentScript(message) {
  window.postMessage({ type: "FROM_WEB_RESOURCE", message: message }, "*");
}
}
